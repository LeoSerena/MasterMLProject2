...
def mask_to_submission_strings(image_filename, pred):
    ...
    for j in range(0, im.shape[1], patch_size):
        for i in range(0, im.shape[0], patch_size):
            ...
            if pred:
                # if we are in prediction mode, we return the raw predition
                label = patch_to_pred(patch)
            else:
                label = patch_to_label(patch)
            yield("{:03d}_{}_{},{}".format(img_number, j, i, label))


def masks_to_submission(submission_filename, *image_filenames, pred = False):
    """Converts images into a submission file"""
    with open(submission_filename, 'w') as f:
        f.write('id,prediction\n')
        for fn in image_filenames[0:]:
            f.writelines('{}\n'.format(s) for s in mask_to_submission_strings(fn, pred))